...
- **Insert-mode cursor shape** — HUME's Insert-mode cursor is always a thin bar; there's no setting for a block. Helix defaults every mode to a block cursor until you configure `[editor.cursor-shape]` yourself, so an out-of-the-box Helix shows a colored block in Insert mode where HUME shows the terminal's own bar — the same look Helix gives you once you set `insert = "bar"` there. A theme's `ui.cursor.insert`/`ui.cursor.primary.insert` color only shows up on a second, simultaneous cursor during multi-cursor Insert-mode editing; a single cursor always renders as the terminal's plain bar.
- **Theme format** — Helix uses TOML with `[palette]` indirection and dot-separated UI scope names. HUME's theme loader reads the same file format, the same color forms (hex, a palette name, or one of the sixteen bare terminal color names like `red`), the same modifier names (`crossed_out`, `underlined`), and the same underline styles (`line`, `curl`, `dotted`, `dashed`, `double_line`), so most Helix themes work in HUME unchanged. A terminal color name resolves to a fixed value from the standard terminal palette rather than to your own terminal's configured color, so a theme reads the same everywhere. One thing a theme can contain isn't supported, though it doesn't stop the rest of the theme from loading: the top-level `rainbow` array, which HUME has no rainbow-bracket feature to read. A theme fails to load outright only when the file itself is invalid TOML, or names an `inherits` parent that doesn't exist or forms a cycle; any other malformed entry is left unstyled and named in `:messages` instead. See [Theme scopes](configuration.md#theme-scopes) for the scopes HUME doesn't read.
