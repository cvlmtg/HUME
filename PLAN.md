...
- [x] **Visual-line movement**: `j`/`k` move by display row (visual line) instead of buffer line when soft wrap is active. `move-down`/`move-up`/`extend-down`/`extend-up` promoted to `EditorCmd` variants in `commands.rs` (need wrap config, `FormatScratch`). `preferred_display_col: Option<u16>` on `Editor` latches the target column on the first j/k press and persists across consecutive presses; reset by any non-vertical command. Falls back to buffer-line movement when `WrapMode::None`. Page/half-page scroll updated to dispatch via `EditorCmd`. Jump list tracking preserved for large vertical jumps.
