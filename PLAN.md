...
- [ ] **Document formatter**: unified iterator that yields `(doc_position, visual_position)` pairs for each grapheme — single source of truth for both rendering and position calculation (inspired by Helix's `DocumentFormatter`). Replaces the current multi-step `wrap_line()` → `display_lines_wrapped()` → `render_content()` pipeline. Subsumes soft wrap, tab expansion, and future annotation/virtual-text interleaving into one pass. Key improvements over current approach: **word-boundary wrapping** (break at whitespace when possible, fall back to character wrap for long tokens), **indentation retention** (continuation lines replicate leading indent up to a configurable max), **configurable wrap indicator** (e.g. "↪" at start of continuation lines). Reduces surface area for rendering/position divergence bugs. **Renderer refactor**: the formatter makes the renderer a dumb consumer of a pre-computed visual layout — `render_content()` and `cursor_screen_pos()` currently duplicate grapheme/column math independently (divergence risk); with the formatter, both consume the same iterator. **Gutter provider model**: the formatter also enables a gutter abstraction — a list of gutter "providers" (line numbers, diagnostics, git signs, breakpoints, fold markers) that each emit a cell per display line, replacing the current monolithic `render_gutter()` that will grow unwieldy as columns are added.
- [ ] **Visual-line movement**: `j`/`k` move by display row (visual line) instead of buffer line when soft wrap is active. Requires document formatter for correct position mapping.
