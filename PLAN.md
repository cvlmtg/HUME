...
- [ ] **Unify no-wrap horizontal scroll**: replace `display_col_in_line()` in `editor/src/core/grapheme.rs` with a call to `format_buffer_line()`, eliminating the independent grapheme→column code path that doesn't account for whitespace indicators or inline decorations.
- [ ] **Visual-line movement**: `j`/`k` move by display row (visual line) instead of buffer line when soft wrap is active. Use `format_buffer_line()` for position mapping.
