...
- [x] **Kitty keyboard protocol** (`src/terminal.rs`, `Editor::kitty_enabled`): probe at startup with `crossterm::terminal::supports_keyboard_enhancement()`; push `DISAMBIGUATE_ESCAPE_CODES | REPORT_EVENT_TYPES` flags when supported; pop unconditionally in `restore()` (harmless no-op on legacy terminals); filter `KeyEventKind::Release` in the event loop; store `kitty_enabled: bool` on `Editor`; add Ctrl+h/l/j/k/w/b one-shot extend bindings gated on `kitty_enabled`. Graceful fallback to legacy encoding when the terminal doesn't support the protocol. No `src/input.rs` needed — crossterm handles protocol details transparently.
