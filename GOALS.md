...
| Editing model | **Select-then-act** (Helix/Kakoune) | Motions create selections, actions operate on them. Design for multiple selections from day one (`Vec<Selection>`). Selections are always inclusive — `anchor == head` is a 1-char selection, never a zero-width point. Extend-selection variants are named commands orthogonal to motion type. Key binding (shift-extends à la Kakoune, or visual-mode toggle à la Helix) is decided at the keymap layer — both models are compatible. Text objects and keystroke macros supported. |
