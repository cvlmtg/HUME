...
| Register `+` (system clipboard) | **Deferred to M3 (editor layer)** | The `+` register requires OS clipboard integration (e.g. `arboard` crate) and belongs in the editor layer, not the core. The `RegisterSet` supports `+` as a key; actual clipboard read/write will be wired in M3. For now it behaves like any named register. |
