...
| Editing model | **Select-then-act** (Helix/Kakoune) | Motions create selections, actions operate on them. Design for multiple selections from day one (`Vec<Selection>`). Selections are always inclusive — `anchor == head` is a 1-char selection, never a zero-width point. Shift-variants of motions extend the selection (Kakoune model). Helix-style visual mode may be added later — both models are compatible with the core design. Text objects and keystroke macros supported. |
