...
`src/lib/vocabulary.generated.js` is also **generated output** — HUME's theme loader's own modifier/underline/ANSI-colour/style-key/cursor-ladder vocabulary and UI chrome scope names, rendered from `hume-engine/src/theme/loader/` and `hume-engine/src/theme/ui_scopes.rs` by a Rust test. Regenerate it after any change to that vocabulary with:
...
- Scopes outside the catalog in `src/data.js` (e.g. from an imported theme with its own invented scopes) are preserved on export but aren't editable in the UI. The catalog is meant to track what HUME resolves; its "UI" category is generated from `hume-engine`'s own `ui_scopes::ALL` (see `src/lib/vocabulary.generated.js`'s banner), but Cursor/Virtual/Diagnostic and everything else is still maintained by hand — `tests/coverage.test.js` checks that every catalog entry outside the UI/Cursor/Virtual/Diagnostic groups is rendered somewhere in the preview and vice versa — those four groups are seeded from the catalog itself, so they pass by construction — and, for the still-hand-maintained groups, nothing checks the catalog against HUME's own scope names.
