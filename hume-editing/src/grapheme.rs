...
pub fn next_grapheme_boundary(buf: &BufferText, char_offset: usize) -> usize {
    hume_rope::grapheme::next_grapheme_boundary(buf.full_slice(), char_offset)
}

/// See [`hume_rope::grapheme::prev_grapheme_boundary`].
pub fn prev_grapheme_boundary(buf: &BufferText, char_offset: usize) -> usize {
    hume_rope::grapheme::prev_grapheme_boundary(buf.full_slice(), char_offset)
}

/// See [`hume_rope::grapheme::grapheme_col_in_line`].
pub fn grapheme_col_in_line(buf: &BufferText, line_idx: usize, char_pos: usize) -> usize {
    hume_rope::grapheme::grapheme_col_in_line(buf.full_slice(), line_idx, char_pos)
}

/// See [`hume_rope::grapheme::display_col_in_line`].
pub fn display_col_in_line(buf: &BufferText, line_idx: usize, char_pos: usize, tab_width: u8) -> usize {
    hume_rope::grapheme::display_col_in_line(buf.full_slice(), line_idx, char_pos, tab_width)
}
