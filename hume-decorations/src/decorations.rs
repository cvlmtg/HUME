...
/// `scope` is interned by `hume-editor`'s `host_impl.rs`'s `set_eol_text` at the
/// `set-eol-text!` boundary.
pub struct EolTextEntry {
    pub pos: CharOffset,
    pub text: String,
    pub scope: ScopeId,
}
...
/// interned by `hume-editor`'s `host_impl.rs`'s `set_extra_highlights` at the
/// `set-extra-highlights!` boundary.
pub struct ExtraHighlightEntry {
...
impl<K, T> SourceStore<K, T> {
    ...
    pub fn for_buffer(&self, bid: BufferId) -> impl Iterator<Item = (&K, &T)> {
...
impl<K, T: Positioned> SourceStore<K, T> {
    ...
    pub(in crate::decorations) fn groups_in_range(
        &self,
        bid: BufferId,
    ...
    pub(in crate::decorations) fn in_range(
        &self,
        bid: BufferId,
        range: ExclusiveRange<CharOffset>,
...
impl DecorationStores {
    ...
    pub fn reset(prior_clock: u64) -> Self {
        Self {
    ...
    /// entries.
    pub fn generation(&self, bid: BufferId) -> u64 {
        self.generation.get(&bid).copied().unwrap_or(0)
    }

    /// Replaces `source`'s inlay hints for `bid` wholesale.
    pub fn set_inlay_hints(&mut self, source: String, bid: BufferId, hints: Vec<InlayHintEntry>) {
    ...
    #[cfg(any(test, feature = "test-util"))]
    pub fn inlay_hints_for_buffer(&self, bid: BufferId) -> impl Iterator<Item = &InlayHintEntry> {
    ...
    pub fn set_signs(&mut self, source: String, bid: BufferId, signs: Vec<SignEntry>) {
        self.signs.set(source, bid, signs);
        self.touch(bid);
    }

    #[cfg(any(test, feature = "test-util"))]
    pub fn signs_for(&self, source: &str, bid: BufferId) -> &[SignEntry] {
    ...
    pub fn register_sign_source(&mut self, name: String, bid: BufferId, priority: i64) {
    ...
    pub fn sign_slot(&self, bid: BufferId, name: &str) -> Option<usize> {
        self.sign_sources
            .get(&bid)?
            .iter()
            .position(|(n, _)| n == name)
    ...
    #[cfg(any(test, feature = "test-util"))]
    pub fn virtual_lines_for(&self, source: &str, bid: BufferId) -> &[VirtualLineEntry] {
        self.virtual_lines.entries_for(source, bid)
    }

    /// Replaces `source`'s EOL text for `bid` wholesale.
    pub fn set_eol_text(&mut self, source: String, bid: BufferId, entries: Vec<EolTextEntry>) {
    ...
    #[cfg(any(test, feature = "test-util"))]
    pub fn line_backgrounds_for(&self, source: &str, bid: BufferId) -> &[LineBgEntry] {
    ...
    #[cfg(any(test, feature = "test-util"))]
    pub fn extra_highlights_for(&self, source: &str, bid: BufferId) -> &[ExtraHighlightEntry] {
    ...
    pub fn has_any(&self, bid: BufferId) -> bool {
        let Self {
            inlay_hints,
            signs,
            virtual_lines,
    ...
    pub fn remove_buffer(&mut self, bid: BufferId) {
        let Self {
            inlay_hints,
            signs,
            virtual_lines,
    ...
    pub fn remap_through(&mut self, bid: BufferId, cs: &ChangeSet) {
        let Self {
            inlay_hints,
            signs,
            virtual_lines,
