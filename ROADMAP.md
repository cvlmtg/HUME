...
- **Splits & pane focus (`:split` / `:vsplit` / pane focus commands)**: stubs already registered; multi-pane scaffolding on `Editor` is split-ready (`LayoutTree::Split`, `EngineView::render` per-pane loop, `Editor::open_pane`/`close_pane`/`switch_focused_pane`). Gaps: none of those mutate `view.layout`; render closures only resolve the focused pane; `wrap_mode` isn't on `Pane`; focus/close stubs return "not yet implemented".
  ...
  - **Sub-tasks**:
    ...
    - [ ] **T3 — Multi-pane render / `prepare_frame`** (`hume-editor/src/editor/lifecycle.rs`): `get_rope` resolves ropes for every pane's `buffer_id`, not just focused; `get_pane_settings` resolves per pane; `prepare_frame` scroll-sync iterates all panes from `collect_rects_into`. Engine insta snapshot of a `Split` rendering both panes at correct rects.
    - [ ] **T4 — Directional pane-focus** (`hume-editor/src/editor/commands/jump.rs`): `cmd_pane_focus_{left,right,up,down}` via `collect_rects_into` + nearest-rect-edge from focused pane; `cmd_pane_focus_next` cycles by tree DFS order. Tests: 2×2 grid; directional focus lands on expected pane; `next` cycles all four.
    - [ ] **T5 — Close-pane command + `Ctrl+p c`**: new native `close-pane` + typed `:close`/`:clo`. If `view.panes.len() == 1` → delegate to `:q`. Else `LayoutTree::remove_leaf(focused)` → switch focus to returned sibling → `close_pane(old_focused)`. Bind `Ctrl+p c` in `build_pane_trie`. Tests: close removes pane + collapses layout; last pane quits.
    - [ ] **T6 — Move `wrap_mode` onto `Pane`** (`hume-engine/src/pane.rs`, `hume-editor/src/editor/lifecycle.rs`, ~80 read sites in cursor/mouse/visual_move/format/layout): add `pub wrap_mode: WrapMode` to `Pane`, seeded from `EditorSettings::wrap_mode` in `Editor::open_pane`; read from `pane.wrap_mode` in the format pipeline; rewrite read sites through `Editor::focused_wrap_mode()` SSOT helper; **remove** `BufferOverrides.wrap_mode` (keep `tab_width`/`whitespace`); migrate affected settings tests. Done after T1–T5 so multi-pane wrap is a real testbed.
    - [ ] **T7 — Keymap + docs**: extend `build_pane_trie` with `s` → `:split`, `v` → `:vsplit`, `c` → `close-pane` (keep `w`/`h`/`j`/`k`/`l`); convert kitty stub tests (`tests/kitty.rs:66-84`) to real-behavior tests; update end-user docs `docs/default_keybinds.md` + `user-manual/docs/commands.md:37` from "not yet implemented" to actual behavior.
