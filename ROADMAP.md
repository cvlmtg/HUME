...
| Close-pane | **`Ctrl+p c` / `:close`; last pane quits** | Prunes the focused leaf, collapses the parent `Split`, refocuses the sibling. Closing the sole pane quits the editor like Vim `:q`. |
| `wrap_mode` scope | **Moved onto `Pane`; `EditorSettings.wrap_mode` is init-default only** | Wrap is a view property, not a document preference. `BufferOverrides.wrap_mode` removed. Runtime per-pane toggle (`set-pane-option!` / `:set wrap`) deferred out of M10. |
...
- **Splits & pane focus (`:split` / `:vsplit` / pane focus commands)**: stubs already registered; multi-pane scaffolding on `Editor` is split-ready (`LayoutTree::Split`, `EngineView::render` per-pane loop, `Editor::open_pane`/`close_pane`/`switch_focused_pane`). Gaps: none of those mutate `view.layout`; render closures only resolve the focused pane; `wrap_mode` isn't on `Pane`; focus/close stubs return "not yet implemented".
  - **Design note — move `wrap_mode` onto the engine `Pane` (`hume-engine/src/pane.rs`) when wiring splits**: wrap is a view property; two panes sharing a buffer at different widths each want their own wrap mode. Today it flows the other way — the engine `Pane` doesn't carry it at all; it's supplied per-frame via `PaneRenderSettings.wrap_mode` from the editor layer. Add `pub wrap_mode: WrapMode` to `Pane`; initialise from `EditorSettings::wrap_mode`; read from `pane_ctx.pane.wrap_mode` in the format pipeline; make `Pane.wrap_mode` the SSOT (see T6). Keep `tab_width` and `whitespace` on `Buffer.overrides` — they are document preferences, not view preferences.
  - **Sub-tasks**:
    ...
    - [ ] **T5 — Close-pane command**: new native `close-pane` + typed `:close`/`:clo`. If `view.panes.len() == 1` → delegate to `:q`. Else `LayoutTree::remove_leaf(focused)` → switch focus to returned sibling → `close_pane(old_focused)`. Keymap binding done in T7. Tests: close removes pane + collapses layout; last pane quits.
    - [ ] **T6 — Move `wrap_mode` onto `Pane`** (`hume-engine/src/pane.rs`, `hume-engine/src/pipeline.rs`, `hume-editor/src/editor/lifecycle.rs`; `wrap_mode` is threaded as a param/read in ~110 places across `hume-editor/src`, fewer true read sites): add `pub wrap_mode: WrapMode` to the engine `Pane`, seeded from `EditorSettings::wrap_mode` in `Editor::open_pane`; update the `Pane`/`PaneRenderSettings` doc comment (wrap moves from "editor-domain fact" to pane-owned; mode/tab_width/whitespace stay per-frame); make `Pane.wrap_mode` the SSOT — drop `PaneRenderSettings.wrap_mode` or seed it from the pane; read from `pane.wrap_mode` in the format pipeline; rewrite read sites through `Editor::focused_wrap_mode()` SSOT helper; **remove** `BufferOverrides.wrap_mode` (keep `tab_width`/`whitespace`); migrate affected settings tests. Done after T1–T5 so multi-pane wrap is a real testbed.
    - [ ] **T7 — Keymap + docs**: split/vsplit half done — `build_pane_trie` now binds `s` → `pane-split`, `v` → `pane-vsplit` (new native `EditorCmd`s sharing `split_pane_onto` with the typed `:split`/`:vsplit`, added in T9); `pane-focus-next` moved from `w` to `p` (frees `w` from any pane-trie ambiguity) — `w`/`h`/`j`/`k`/`l` still hold focus movement. Kitty tests updated to real-behavior (`ctrl_p_p_is_noop_with_single_pane`, `ctrl_p_s_splits_pane`, `ctrl_p_v_vsplits_pane`); end-user docs (`docs/default_keybinds.md`, `user-manual/docs/commands.md`, `user-manual/docs/key-reference.md`, `user-manual/docs/roadmap.md`) updated off "not yet implemented". Still open: `c` → `close-pane` (blocked on T5).
    - [ ] **T8 — Disjoint-borrow lint skeleton** (`hume-editor/src/editor/lints.rs`): narrow — document the rule (free functions over `&mut self` facility methods, per `ROADMAP.md:45`) and forbid NEW `&mut self` facility methods on `Editor` beyond an allowlist (`open_pane`/`close_pane`/`switch_focused_pane`/`render_into`). No refactor of existing methods. Deep audit deferred to its own roadmap entry.
    - [x] **T9 — Pane seam dividers + focus dimming** (`hume-engine/src/pipeline.rs`, `hume-engine/src/render.rs`, `hume-editor/src/editor/commands/typed_misc.rs`): `split_rect` reserves a 1-cell seam between sibling panes (`LayoutTree::collect_seams_into`), drawn `│`/`─` and accent-colored when touching the focused pane (`seam_touches_rect`), muted otherwise. Non-focused panes are true-color-dimmed toward `ui.background` (`render::dim_rect`, not `Modifier::DIM` — inconsistent terminal support). `:split`/`:vsplit` reject with a status warning (no layout mutation) when the focused pane is too small to fit two panes plus the seam (`fits_split`, `MIN_PANE_WIDTH`/`MIN_PANE_HEIGHT`). New `pane-dividers` setting (default on). Added `ui.background`/`ui.window`/`ui.window.focused` to `dark.toml` (`ember.toml` already had `ui.background`). Statusline stays a single global bar — no per-pane statusline. Done ahead of T5–T8 (independent of close-pane/wrap_mode/keymap/lint work).
  - **Implementation order**: T1 → T2 → T3 → T4 → (parity snapshot) → T5 → T6 → T7 → T8. T6 late so multi-pane wrap is a real testbed. T9 done out of sequence, directly after T4 (only depends on the T4 rect cache); T7's split/vsplit keybind + doc portion done alongside T9, `close-pane` binding still deferred to real T7 (needs T5).
