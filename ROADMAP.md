...
- **Splits & pane focus (`:split` / `:vsplit` / pane focus commands)**: stubs already registered; multi-pane scaffolding on `Editor` is split-ready (`LayoutTree::Split`, `EngineView::render` per-pane loop, `Editor::open_pane`/`close_pane`/`switch_focused_pane`). Gaps: none of those mutate `view.layout`; render closures only resolve the focused pane; `wrap_mode` isn't on `Pane`; focus/close stubs return "not yet implemented".
  ...
  - **Sub-tasks**:
    ...
    - [ ] **T7 — Keymap + docs**: split/vsplit half done — `build_pane_trie` now binds `s` → `pane-split`, `v` → `pane-vsplit` (new native `EditorCmd`s sharing `split_pane_onto` with the typed `:split`/`:vsplit`, added in T9); `pane-focus-next` moved from `w` to `p` (frees `w` from any pane-trie ambiguity) — `w`/`h`/`j`/`k`/`l` still hold focus movement. Kitty tests updated to real-behavior (`ctrl_p_p_is_noop_with_single_pane`, `ctrl_p_s_splits_pane`, `ctrl_p_v_vsplits_pane`); `c` → `pane-close` bound in T5. Still open: end-user docs (`user-manual/docs/commands.md`, `user-manual/docs/key-reference.md`, `user-manual/docs/roadmap.md`) updated off "not yet implemented" — `docs/default_keybinds.md` kept current incrementally through T2/T5.
    - [ ] **T8 — Disjoint-borrow lint skeleton** (`hume-editor/src/editor/lints.rs`): narrow — document the rule (free functions over `&mut self` facility methods, per `ROADMAP.md:45`) and forbid NEW `&mut self` facility methods on `Editor` beyond an allowlist (`open_pane`/`switch_focused_pane`/`render_into`). No refactor of existing methods. Deep audit deferred to its own roadmap entry.
  ...
  - **Implementation order**: T1 → T2 → T3 → T4 → (parity snapshot) → T5 → T6 → T7 → T8. T6 late so multi-pane wrap is a real testbed. T9 done out of sequence, directly after T4 (only depends on the T4 rect cache); T7's split/vsplit keybind + doc portion done alongside T9, `pane-close` binding done directly in T5 (not deferred to T7 after all).
