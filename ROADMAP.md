...
- **Splits & pane focus (`:split` / `:vsplit` / pane focus commands)**: stubs already registered; multi-pane scaffolding on `Editor` is split-ready (`LayoutTree::Split`, `EngineView::render` per-pane loop, `Editor::open_pane`/`close_pane`/`switch_focused_pane`). Gaps: none of those mutate `view.layout`; render closures only resolve the focused pane; `wrap_mode` isn't on `Pane`; focus/close stubs return "not yet implemented".
  ...
  - **Sub-tasks**:
    ...
    - [ ] **T7 — Keymap + docs**: extend `build_pane_trie` with `s` → `:split`, `v` → `:vsplit`, `c` → `close-pane` (keep `w`/`h`/`j`/`k`/`l`); convert kitty stub tests (`hume-editor/src/editor/tests/kitty.rs:53-109` — `ctrl_p_starts_pane_prefix`, `ctrl_p_w_is_pane_focus_next_stub`, `ctrl_p_directional_stubs_report_not_implemented`) to real-behavior tests; update end-user docs `docs/default_keybinds.md` + `user-manual/docs/commands.md:37` from "not yet implemented" to actual behavior.
  ...
  - **Implementation order**: T1 → T2 → T3 → T4 → (parity snapshot) → T5 → T6 → T7 → T8. T6 late so multi-pane wrap is a real testbed. T9 done out of sequence, directly after T4 (only depends on the T4 rect cache).
