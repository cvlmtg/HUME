...
1. ~~**No fuzzy matcher**~~ — **done (B1)**: `nucleo-matcher` 0.3.1 + `hume-editor/src/editor/fuzzy.rs` (`FuzzyMatcher`/`FuzzyPattern`). Completion's filter is still hand-rolled subsequence-only (see `docs/COMPLETION-PICKER.md`) and stays that way per Q-B6.
2. ~~**No input-box-plus-filtered-list widget**~~ — **done (B3)**: `hume-editor/src/ui/picker_panel.rs` (`PickerViewState`, `PickerOverlay`, `panel_geometry`, `draw_picker_panel`). Menu and drawer still take static pre-filtered lists; `prompt!` still has no list — this widget is the picker's own, not a retrofit onto either.
...
4. ~~**No modal input+list widget with unbounded scrolling tuned for large result sets.**~~ — **done (B3)**: `PickerSession`'s edge-anchored scroll (B2) paired with `picker_panel`'s fixed-size box — a query input line driving live re-ranking, windowed independently of item count, distinct from `menu_box`'s selection-centered `visible_window`.
...
**B3 — widget + interaction.** New centered overlay (e.g. `ui/picker_panel.rs`): bordered panel sized as a fraction of the *panes region* (the terminal minus chrome bands — see Q-B2) — say width `min(80%, 100 cols)`, height `min(60%, 30 rows)`; input line at top (rendered from `query` + a block cursor cell), ranked list below with `selected` highlighted and real scrolling. Theme scopes reuse Helix's own picker-body scopes rather than inventing new ones: `ui.background` (panel fill), `ui.text` (border/rows/query), `ui.text.focus` (selected row — Helix's docs call it "the currently selected line in the picker"), `ui.cursor.primary` (query block cursor). `Theme::resolve_raw`'s prefix-trim fallback (`ui.text.focus` → `ui.text` → default) is exactly what Helix relies on too, so no custom aliasing layer is needed. Rendering follows the universal write-side/read-side split: `sync_picker_view` in `prepare_frame` writes an `Arc<RwLock<Option<PickerViewState>>>`; a new `OverlayProvider` only paints (per-pane registration suffices — overlays receive the whole panes region, Q-B2).
...
**Default keybindings**: `g f` → `picker-files`, `g b` → `picker-buffers`, bound under the native `g` trie the same way `core:lsp` extends it (`bind-key! 'normal "g f" "picker-files"` etc. — see the "Picker trigger prefix" Decisions row below). Plugin-added, not a `keymap/defaults.rs` change. `g` has room for these once `core:lsp`'s `lsp-references`/`lsp-hover`/`lsp-code-actions` relocate to `z` (`z r`/`z k`/`z a`) — a companion keybind change made alongside this decision, freeing the response/action-shaped LSP letters `g` was using while the jump-shaped ones (`g d`/`g D`/`g y`/`g i`/`g r`/`g n`/`g p`) stay put. No `symbols` picker binding yet — out of B6's scope (files + buffers only); workspace symbols is a separate, unscheduled LSP-v1 non-goal noted above.
...
| B3 | ~~Panel widget + view sync + key interception; insta snapshot tests + interaction tests~~ **DONE** | B2 | M–L (largest single piece — new chrome surface) |
| B4 | ~~Steel builtins `picker!`/`picker-push!`/`picker-close!` + host trait/impl + callback firing; tests incl. stale-token push~~ **DONE** | B3 | M |
...
| Picker trigger prefix | **`g` (`g f` files, `g b` buffers)** | `Ctrl+p` is already the pane prefix and separately reused for in-picker up/down nav per B3 — doubling it as the trigger would be confusing even though the contexts don't technically collide. `Space` is deliberately not a HUME prefix (unlike Kakoune's leader) — kept free for user config. `g` has room once `core:lsp`'s `lsp-references`/`lsp-hover`/`lsp-code-actions` move to `z` (`z r`/`z k`/`z a`, a companion keybind change — see `runtime/plugins/core/lsp/plugin.scm`), leaving the jump-shaped LSP commands and pickers to share `g` without crowding. |
| B1 fuzzy-matcher budget | **`nucleo-matcher` (not full `nucleo`); B1's 100k-item budget test bound is 32ms (2 frames), not 16ms (1 frame)** | Measured single-threaded scoring+ranking of 100k paths against a short low-selectivity query: stable ~15.5-16.5ms release-mode across 7 runs — right at one frame's edge. A single dropped frame on the heaviest realistic keystroke is imperceptible to a typing user; escalating to full `nucleo` (background/incremental matching, Q-B1's documented fallback) would reshape B2's store design before it's even built, for a gain not yet shown to be needed. |
| `Editor::open_picker` chokepoint (B3) | **Built in B3 as the single open path; relocated to a state-level free fn in B4** — `picker::open_picker(state, lsp, session)` / `picker::close_picker(state, payload)`, not `Editor` methods. `EditorHostImpl` (B4's production caller) holds disjoint `state`/`lsp` borrows, never a whole `Editor`, so it can't call an `Editor` method — the chokepoint had to move down to the borrows both callers actually have. Q-B7's "close a live completion session on open" and "replacing an open picker fires its `on_select` with `#f`" both still live at this one place; `close_picker` is also the shared fire point for `Esc`, `Enter`, and `picker-close!`. | The exactly-once callback contract should never have a window where a replaced or closed session's callback can be silently dropped. A single free fn reachable from both `Editor`-based tests and `EditorHostImpl` keeps that guarantee without duplicating it per caller (LESSONS.md L2). |
| `picker-push!` return value | **Returns `#t`/`#f`** (applied vs. stale-token-or-no-picker) | Doc was silent on this. The store's `push` already returns `bool`; an async source (B5) wants to know whether it can stop pushing to a picker the user already closed, without needing a separate liveness check. |

## Open questions

Each carries a `Default:` per the usage rules.

**Q-B1 (resolved — see Decisions table above)** — `nucleo-matcher` vs full `nucleo`. Measured: scoring+ranking 100k synthetic paths against a short low-selectivity query costs a stable ~15.5-16.5ms release-mode (7-run spread, single machine) — right at one 16ms frame, under two. Resolved as `nucleo-matcher` + a 2-frame (32ms) budget rather than escalating to full `nucleo`'s background/incremental matching, which would have reshaped B2's store before it exists. Revisit only if real usage shows the picker's worst-case keystroke feels laggy.
