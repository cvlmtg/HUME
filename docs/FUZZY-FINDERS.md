...
**Wake is reuse, not new machinery** (correcting this doc's 2026-07-10 warning, stale as of 2026-07-13): `AsyncSource`/`wake_timeout` now generalize the event-loop wake — parse worker, timer wheel, and LSP are the three current sources, and the LSP impl already models "in flight → poll at `PENDING_POLL` (8ms)". A spawn source is a fourth entry in `Editor::async_sources` plus a `next_wake` impl returning `PENDING_POLL` while any child is live. The file's own comment says it: "Adding a source = one line here plus its `AsyncSource` impl."
