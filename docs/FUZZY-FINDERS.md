...
| `Editor::open_picker` chokepoint (B3) | **Built now, in B3, as the single open path** — Q-B7's "close a live completion session on open" and B4's specified "replacing an open picker fires its `on_select` with `#f`" both live here, ahead of B4's `picker!` builtin existing. B4 calls this same function; it does not re-implement either behavior. | The exactly-once callback contract should never have a window — even a B3-only, test-driven window — where a replaced session's callback can be silently dropped. Landing both rules at the one chokepoint now (L2-style) means B4 only has to parse `picker!`'s arguments and call `open_picker`, not re-derive replace semantics. |
