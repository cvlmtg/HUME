...
| Virtual-line-aware scroll/cursor math | **Landed end to end; wrap-mode paths only** | `hume-editor`'s `cursor::screen_pos`/`screen_to_char_offset` and `scroll::ensure_cursor_visible`/`scroll_cursor_to_row`/`scroll_backward_from_cursor` use `format::display_rows_for_line`'s `RowsBreakdown` instead of content-only `count_visual_rows` — mechanism fully documented at those definitions and in `scroll.rs`'s "viewport's own top line never shows its `before` block" comments. **Scope caveat, not stated in source**: this is scoped to the *wrapping* code path only — `ensure_cursor_visible_unwrapped` (no-wrap vertical scroll) and `ensure_cursor_visible_horizontal`/`format_row_col` (column math, never cross-line) stay untouched, so a virtual line's effect on no-wrap vertical scrolling remains unhandled. |
