...
| Multiple servers per language | **Last-wins: a second `register-lsp-server!` for a language *replaces* its config (matching `define-language!`), rather than being rejected.** Running clients on the old config are left untouched until their next spawn — a caller needing an immediate respawn (e.g. reinstalling a server) unregisters explicitly first via `unregister-lsp-server!`. Both builtins are queued and applied via `Editor::apply_lsp_server_ops` at the end of an init eval or a command/hook body — see `docs/LSP-INSTALL.md` step 2. **Caveat (2026-07-13, see `docs/ROADMAP.md`'s "Lazy-activation queued effects" open question)**: a *lazy* plugin activation (`#:languages`/`#:events`) does not itself trigger this flush — a registration queued from such a plugin's own top level is only applied at the next hook-with-a-handler fire or command dispatch, not synchronously within the activating call. | Changed in LSP-INSTALL step 2 (`hume-editor/src/editor/lsp/registry.rs`'s `apply_pending_lsp_server_reg`/`apply_lsp_server_op`). The diagnostics store already keys by `(server, buffer)`, so the door stays open for a future per-buffer override without a data-model change. **Deliberate, reaffirmed by 2026-07-13 code review**: a hard error on re-registration would make user config unable to win over a plugin's default — `init.scm` loads after every plugin, so a user's own `register-lsp-server!` for a language a plugin already registered (e.g. overriding a plugin's `pyright` default with `pylsp`) must silently replace it, not be rejected. The replacement is logged at `Severity::Trace` (visible via `:messages`, not surfaced as a warning) since last-wins is the intended, unsurprising outcome, not a conflict. This is not a missing duplicate-registration guard — do not add one back. |

| `(declared-plugins)` scope (quality-review follow-up, 2026-07-14) | **Reports every declared plugin, `core:*` included** | Originally widened so `plum/notify-lsp!` could warn "core:lsp is not loaded" only for a truly absent `core:lsp`, distinguishing it from a correctly, lazily declared one (the builtin previously filtered out `core:*` names, and `loaded-plugins` excludes anything not yet activated, so no Steel-visible way existed to tell "declared but inactive" from "never configured"). The never-install-core filter moved to where it actually belongs — PLUM's own install-list logic (`plum/missing-plugins` in `plugins.scm`), since core plugins being un-installable is PLUM's policy, not a property of what init.scm declared. **Revised 2026-07-14**: `plum/notify-lsp!` and its three-way dispatch no longer exist — LSP server install/uninstall/registration moved wholesale into `core:lsp` (see `docs/LSP-INSTALL.md`'s Placement section), so install and registration are a direct same-plugin call, no cross-plugin notify to route. The widened builtin is kept as-is (generic, harmless introspection; `plum/missing-plugins` still re-filters `core:*` locally) rather than narrowed back — no code depends on the wider scope today, but narrowing it buys nothing either. |
