...
| LSP idle wake (C4) | **Superseded — see ROADMAP.md's "Event-loop waker" row** | Originally an 8ms poll while any client was Starting or had a request in flight, plus a 200ms Running-idle heartbeat, since `event::read()` couldn't be woken externally. Replaced by a real cross-thread waker (`hume_platform::events`): the LSP transport threads signal the main loop's wait primitive directly after posting a result, so `LspState::next_wake` now reports real deadlines only (the earliest pending-request deadline, plus the statusline spinner's cadence while Starting/`$/progress`) — no poll cadence, no heartbeat. |
...
| `additionalTextEdits`/`completionItem/resolve` ownership (R2, post-v1 refinement) | **Moved from Steel to Rust — atomic accept, exact resolve position mapping** | Originally Steel's `on-completion-accept` handler applied `additionalTextEdits` itself, shifting same-line entries past the main edit by a hand-computed UTF-16 length delta (an approximation — only handled one line, one main edit). `CompletionSession::accept` (`hume-editor/src/editor/lsp/completion.rs`) now batches the main edit and any `additionalTextEdits` into one `apply_text_edits_returning_cs` call — all wire positions computed by the server against the *same* pre-accept document, so no relative adjustment between them is needed at all. When the item lacks `additionalTextEdits` entirely (not just an empty array) and the server advertises `resolveProvider`, accept sends `completionItem/resolve` synchronously (through the ordinary `LspCallback`/`stale_check` machinery every `lsp-request` uses, so `:lsp-stop`'s existing `drain_pending` teardown sweeps it for free — no bespoke queue). The response's positions are decoded against the pre-accept rope, then mapped *forward* through the accept edit's own `ChangeSet` (`ChangeSet::map_ranges`) onto the buffer as it now stands — exact tracking through the intervening edit, not a delta approximation. |
...
| `workspace.workspaceEdit` client capability (F5 platform gap) | **Declared: `documentChanges`, `resourceOperations` (create/rename/delete), `failureHandling: Abort`** | F5's manual smoke test found rust-analyzer refuses `textDocument/rename` outright (`"Client does not support rename capability."`) without this — every rename result is a `WorkspaceEdit`, and the server can't confirm the client can apply one otherwise. `hume-lsp/src/client.rs`'s `build_client_capabilities()` already declared `text_document.rename` but never `workspace.workspace_edit`; a Rust change outside Step 4's zero-Rust scope, approved before landing (mirrors B10's precedent — a one-off platform-gap fix, not a recurring pattern, so it stayed inline rather than becoming its own numbered card). **`resourceOperations` specifically must stay non-empty** — live-verified twice: omitting it blocks *every* rename (not just file-touching ones); declaring it lets a rename whose target shares its name with its containing module/file occasionally return a `DocumentChangeOperation::Op` HUME has no equivalent for, which `edits::collect_edit_entries` (B6) already rejects atomically (safe, pre-existing design — no new gap). Cross-file renames from a real definition site confirmed working end-to-end (2-buffer `WorkspaceEdit` applied correctly). |
| Wire codec null-result bug (F9 platform gap, not a design decision) | **Fixed in `hume-lsp/src/codec.rs`: `result`/`error` distinguish "field absent" from "field present as JSON `null`"** | F9's manual smoke test found rust-analyzer's spec-valid `{"id":N,"result":null}` reply to `workspace/executeCommand` crashed the whole connection ("server crashed: message body is neither request, response, nor notification"). `serde`'s default `Option<Value>` deserialize collapses null-present and field-absent to the same `None`, so `classify()`'s `Some(result)` match arm for a successful `Response` never matched a null payload — this affects *any* LSP method that can legitimately succeed with no payload (hover-with-no-info, goto-with-no-definition, empty code actions, already-formatted, nothing-to-rename, executeCommand, …), not just F9. Invisible to every prior unit test because `InlineLspBackend` constructs `Message`s directly in Rust, bypassing the wire codec — only a real server on the real wire exercises `classify()`. A genuine bug in already-shipped C2 code, not a gap requiring a design call; approved and fixed before landing F9, same zero-Rust-in-Step-4 exception as the two rows above. |
| `map_ranges` crash on nested/overlapping ranges (found via F9, not an LSP-specific bug) | **Fixed in `hume-editing/src/changeset/mod.rs`: ends walked via an index permutation sorted by end value, not input order** | Applying a real rust-analyzer quickfix crashed the whole editor ("attempt to subtract with overflow"). `ChangeSet::map_ranges` only asserted its input sorted by *start*, but walked ends through a resumable cursor in that same order, silently assuming ends were also non-decreasing — true for disjoint/overlapping ranges, false for *nested* ones. rustc published an outer "note" diagnostic spanning the whole `use` line and the actual `unused_imports` warning nested inside it; sorted by start, their ends ran backwards, breaking the cursor's forward-only contract. This is core `hume-editing` code (Step 0), not LSP-specific — any caller batching nested ranges through one `map_ranges` call would hit it; `DiagnosticsStore::remap_through` (B5, this session) is the first caller whose ranges can legitimately nest (an LSP server's own diagnostics commonly do). Fixed with a regression test reproducing the exact nested shape; no caller-visible contract change. |
| `textDocument.codeAction` client capability (F9 platform gap) | **Declared: `codeActionLiteralSupport` (quickfix/refactor/source kinds), `isPreferredSupport`, `disabledSupport`** | F9's manual smoke test found rust-analyzer withheld every diagnostic-derived quickfix — a byte-perfect `textDocument/codeAction` request (correct overlapping range, the exact diagnostic round-tripped verbatim via the `raw` field below) still came back empty. `hume-lsp/src/client.rs` declared `code_action: Some(CodeActionClientCapabilities::default())` — every field unset, including the flag telling a server the client understands `CodeAction` literals rather than only legacy `Command[]`. Same zero-Rust-in-Step-4 exception as the two rows above. |
...
| Snippet completions (F3) | **Strip to plain text — confirmed acceptable; Rust-side since R2b** | `insertTextFormat == 2` items get `${n:default}` rewritten to `default` and bare `$n` dropped at store ingress (`StoredCompletionItem::from_typed`/`from_json_lenient`, hume-editor/src/editor/lsp/completion.rs's `strip_snippet`) — no choices, no nested placeholders, no `\$` escapes. Moved from a Steel character-scanning helper originally in `completion.scm` to keep text-content transformation off the Steel side; `raw` (and so `on-completion-accept`/`completionItem/resolve`) now carries the pristine, unstripped snippet text — more spec-correct for resolve, since a server may echo/consume the original snippet syntax. Automated tests (`strip_snippet` unit tests + `snippet_item_lands_as_stripped_plain_text` e2e, flip-checked) prove the exact for-loop-template shape (`for ${1:x} in ${2:iter} {\n    $0\n}`) strips correctly; manual rust-analyzer smoke confirmed accepted completions insert clean text with no stray `$`/`{`/`}` artifacts. Full snippet support (tabstop navigation, multi-cursor placeholder selection) stays Future — the insert-mode state machine for that is Rust, not Steel. |
...
| Server crash policy | **Manual `:lsp-restart [language]` only — no auto-restart** | Shipped in C10, then moved from a Rust typed command into a `core:lsp` Steel command over an `lsp-restart!` builtin (queued onto `PendingLspServerOp`, applied by `Editor::apply_lsp_server_ops`) — see the "LSP server lifecycle ownership" row in `docs/ROADMAP.md`. Stops then respawns through the same C8 spawn path. Revisit bounded auto-restart with real usage data; the policy knob would live in Steel either way. |
| `workspace/configuration` flow | **`register-lsp-server!`'s `#:settings` blob pushed once as `didChangeConfiguration` after `initialized`, and resolved per-item (dotted `section` walked into the blob — VS Code semantics: no section returns the whole blob, a miss returns `null`) to answer `workspace/configuration` pull requests** | Resolution and push live in `hume-lsp/src/client.rs` (`resolve_config_section`, `workspace_configuration_response`, the push in `handle_initialize_response`); the editor glue resolves the requesting server's own config (`server_id -> introspect::server_language -> LspState.configs`) in `hume-editor/src/editor/lsp/mod.rs`'s `ServerRequest` dispatch arm and `registry.rs`'s spawn branch. Not "verbatim" as originally planned — a server asking for a specific `section` gets that sub-object, not the whole blob, matching how real servers (gopls, rust-analyzer) actually request their config. A miss (e.g. gopls/rust-analyzer's own catalog blobs, which aren't nested under their own name) is expected and harmless, same as in Helix — no per-server catalog audit needed; see `docs/ROADMAP.md`'s LSP section. Real configuration for these seeded servers arrives via `#:init-options` (the catalog's `config` field, delivered both ways by `core:lsp/registration.scm` — see ROADMAP), not this pull path. |
| Multiple servers per language | **Last-wins: a second `register-lsp-server!` for a language *replaces* its config (matching `define-language!`), rather than being rejected.** Running clients on the old config are left untouched until their next spawn — a caller needing an immediate respawn (e.g. reinstalling a server) unregisters explicitly first via `unregister-lsp-server!`. Both builtins are queued and applied via `Editor::apply_lsp_server_ops` at the end of an init eval or a command/hook body — see `docs/LSP-INSTALL.md` step 2. A *lazy* plugin activation (`#:languages`/`#:events`/`#:commands`) applies its own queued registration synchronously too — `activate_and_register` (`hume-editor/src/editor/mappings/lazy.rs`) runs `apply_script_effects` before returning, so the buffer whose language-set (or command/event) triggered activation attaches in that same call, with no need to wait for a later drain — see `docs/ROADMAP.md`'s "Lazy-activation side effects" decision row. | `hume-editor/src/editor/lsp/registry.rs`'s `apply_pending_lsp_server_reg`/`apply_lsp_server_op`. The diagnostics store already keys by `(server, buffer)`, so the door stays open for a future per-buffer override without a data-model change. **Rejected: a hard error on re-registration** — it would make user config unable to win over a plugin's default; `init.scm` loads after every plugin, so a user's own `register-lsp-server!` for a language a plugin already registered (e.g. overriding a plugin's `pyright` default with `pylsp`) must silently replace it. The replacement is logged at `Severity::Trace` (visible via `:messages`, not surfaced as a warning) since last-wins is the intended, unsurprising outcome, not a conflict. This is not a missing duplicate-registration guard — do not add one back. |
...
- **Multiple servers per language / per buffer** — Rust routing + result merging. Verified current shape (2026-07): `LspState.servers: HashMap<ServerId, ServerEntry>` is already process-identity-keyed, not language-keyed — a mechanical "re-key by server identity" audited during the R2-batch review turned out to already be the status quo, no code change needed. What *is* language-keyed today: `LspState.configs: HashMap<LanguageName, LspServerConfig>` (one registration per language — a second `register-lsp-server!` for the same language replaces it, see the row above) and each running `ServerEntry.language: Option<LanguageName>` (one language per server). Confirmed: two languages registered under the same `command` already spawn two independent processes (`lsp_attach_buffer`'s `(language, root)` match never reuses a running entry across languages) — so per-server tracking already doesn't assume one process per command. The concrete gap for real multi-server-per-language support: registration identity is language, 1:1 with a config and a running server; landing this needs (a) a registration identity distinct from language (a name, with a language list), (b) `ServerEntry.language` → a registration name + a `LanguageName -> [registration name]` map, (c) `resolve_server`/`lsp_targets` (`hume-editor/src/editor/lsp/introspect.rs`, `registry.rs`) accepting a registration name in addition to a language, (d) the Steel API staying language-keyed and returning singleton lists until a caller needs otherwise. `LanguageName` (`registry.rs`) is a plain `String` alias marking every language-keyed signature in `registry.rs`/`introspect.rs`/`mod.rs` today — a grep starting point for this future work, not a behavior change.
