...
| `(declared-plugins)` scope (quality-review follow-up, 2026-07-14) | **Reports every declared plugin, `core:*` included** | Post-review of the PLUM→core:lsp registration move: `plum/notify-lsp!` warned "core:lsp is not loaded" even for a correctly, lazily declared `core:lsp` (the manual's recommended setup), because the builtin filtered out `core:*` names and `loaded-plugins` excludes anything not yet activated — no Steel-visible way existed to tell "declared but inactive" from "never configured." Widened the Rust builtin (`hume-scripting/src/builtins/plugins.rs`) instead of adding a new one; moved the never-install-core filter to where it actually belongs — PLUM's own install-list logic (`plum/missing-plugins` in `plugins.scm`), since core plugins being un-installable is PLUM's policy, not a property of what init.scm declared. `plum/notify-lsp!` (`servers.scm`) is now a three-way dispatch: loaded → rescan; declared-but-inactive → info note that registration happens on activation; neither → the original warn. |
