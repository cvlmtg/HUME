...
| Hooks | `hume-scripting/src/hooks.rs` — `HookId` enum + `HOOKS` const table (the SSOT: variant ↔ Steel name); editor side `editor/scripting_setup.rs` — `queue_event` enqueues into `state.config.pending_events`, `drain_events` evals with a cascade cap | B7 adds four hooks: extend `HookId` + `HOOKS`, add fire sites, done |
