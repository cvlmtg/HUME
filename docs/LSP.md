...
This file is the **hub**: architecture, decisions, shared reference (protocol primer, codebase orientation, Steel API index, testing playbook). All 48 implementation tasks (P1–P8, C1–C10, B1–B10, U1–U9, F1–F11) shipped; their per-task cards are gone — this doc absorbed everything load-bearing from them. Where the shipped design actually lives:
...
| Snippet completions | **Strip to plain text, Rust-side** | `insertTextFormat == 2` items get `${n:default}` rewritten to `default` and bare `$n` dropped at store ingress (see `strip_snippet` in `hume-editor/src/editor/lsp/completion.rs`) — no choices, no nested placeholders, no `\$` escapes. `raw` keeps the pristine snippet text for `completionItem/resolve`. Full snippet support (tabstop navigation) stays Future. |
| Multi-character inline inserts | **`hume-engine/src/format.rs`: one `Grapheme`/`Cell` per grapheme cluster of the insert text, not one cell for the whole string** | ratatui writes one cell per terminal column; a symbol wider than one cell must be split into one `Cell` per grapheme or its spillover gets overwritten. Full rationale: `format.rs`'s doc comment at the push site. |
...
| Render providers | `hume-engine/src/providers.rs` — `HighlightSource` (line-relative byte spans, tier-layered: `HighlightTier::{Syntax, Extra, SearchMatch, Diagnostic, BracketMatch}`), `VirtualLineSource`, `InlineDecoration` (per-line `InlineInsert`s), `OverlayProvider` (raw ratatui cell access), `ProviderSet::{add_*, remove}`; signs in `hume-engine/src/builtins/sign_column.rs` (`Sign`, `SignSource`, `SignColumn`) | One provider per decoration kind, fed by the Rust-side stores |
...
| Surface | Kind | Task |
|---------|------|------|
| `(register-lsp-server! lang #:command cmd #:args '(…) #:root-markers '(…) #:init-options blob #:settings blob #:env '(("K" . "V")…))` | builtin, last-wins, queued (applies end-of-eval); `#:env` applied additively to the spawned process's inherited environment | C8, LSP-INSTALL step 2, `core:steel-server`'s `STEEL_LSP_HOME` wiring |
...
| `(show-popup! text #:anchor 'cursor #:kind 'sticky #:lang #f)` / `(close-popup!)` — `#:kind` selects the dismiss behavior: `'sticky` (default) closes only via `close-popup!`/`on-mode-change`; `'scrollable` has Ctrl+u/Ctrl+d page the content when it overflows one screenful, any other key — and Ctrl+u/d with nothing to page — closing it (scrollable hover, `gn`/`gp`'s overlay). `#:lang`, when a grammar by that name is registered, syntax-highlights `text` like a real buffer; absent grammar or `#f` renders plain text | builtin | U4, U8 |
...
| `(get-option [bid] key)` → the effective value (buffer override, else global), typed `bool`/`int`/`string` per the setting's parser kind; `Err` on an unknown key | builtin | B10b |
| Settings knobs (`lsp.request-timeout-ms`, `lsp.diagnostics-severity-floor`, `lsp.inlay-hints` (default `false`), …) | via existing `:set` / `set-option!` / `get-option` | various |
...
Workspace groundwork with no LSP-visible behavior. All prerequisite tasks shipped (serde/serde_json deps, batch position mapping, generalized event-loop wake, position encoding conversion, path↔URI, `ChangeSet` conversion, timer wheel, boundary-cost spike).

## Step 1 — LSP core (Rust data plane)

The `hume-lsp` crate plus editor glue: spawn, handshake, document sync, diagnostics in memory. All core tasks shipped (crate scaffold, JSON-RPC codec, process management, `LspBackend` trait + inline double, lifecycle, request bookkeeping, document sync, server registration, diagnostics store, observability/lifecycle commands).

## Step 2 — Steel platform

The bridge and primitives that make Steel the feature layer — a plugin can reach any LSP method and any editor surface. All platform tasks shipped (JSON↔SteelVal codec, generic LSP bridge, introspection builtins, timers, decoration stores, edit/navigation primitives, hooks, completion orchestration, minibuffer prompt).

## Step 3 — UI surfaces

Generic, Steel-scriptable widgets plus store-fed render wiring — LSP is their first client, not their owner. All UI-surface tasks shipped (diagnostic underlines, gutter signs, statusline element, popup, menu, drawer, completion menu, inline diagnostics, inlay hints).

## Step 4 — Features (Steel, `core:lsp`)

Each feature is Steel code composing the platform and UI primitives above, tested against the inline double with scripted responses. All feature tasks shipped (hover, goto definition family, completions, diagnostics navigation, rename, references, signature help, formatting, code actions, inlay hints, `core:lsp` packaging + docs).
