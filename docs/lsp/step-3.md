...
**Files** — `ui/popup.rs` (completion menu = U5 menu + doc column), Insert-mode key routing (`editor/mappings/` insert path), `keymap/defaults.rs` (`Ctrl+Space` binding → a `completion-trigger` **named command** so plugins can rebind), builtins already exist (B8).
