...
> **Superseded for diagnostics.** U8a (the scroll/cursor `ProviderSet` rewiring) and the generic `VirtualLineSource`/`set-virtual-lines!` plumbing described below landed as planned and remain live infrastructure (see the ROADMAP decision row) — but U8b's diagnostics-specific consumer (a `set-virtual-lines!` call from `on-diagnostics-changed`) was never written, and the roadmap checkbox marking U8 done was wrong on that point. Diagnostics' actual inline summary instead reuses U9's `InlineDecoration` shape: end-of-line text via a second `InlayHintProvider` instance fed by a new `set-inline-diagnostics!` store, plus a `gn`/`gp` dismiss-on-next-key popup (`show-popup! #:dismiss-on-key`) for the full message. See `docs/LSP.md`'s U8 entry and `runtime/plugins/core/lsp/diagnostics.scm`. The `VirtualLineSource` path below remains available for a future under-the-line feature (git blame, code lens) that actually wants whole synthetic rows.
