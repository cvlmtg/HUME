...
> **Diagnostics use a different mechanism than this card's U8b.** U8a (the scroll/cursor `ProviderSet` rewiring) and the generic `VirtualLineSource`/`set-virtual-lines!` plumbing described below are live infrastructure (see the ROADMAP decision row), but diagnostics' actual inline summary reuses U9's `InlineDecoration` shape instead: end-of-line text via a second `InlayHintProvider` instance fed by `set-inline-diagnostics!`, plus a `gn`/`gp` dismiss-on-next-key popup (`show-popup! #:dismiss-on-key`) for the full message. See `docs/LSP.md`'s U8 entry and `runtime/plugins/core/lsp/diagnostics.scm`. The `VirtualLineSource` path below remains available for a future under-the-line feature (git blame, code lens) that wants whole synthetic rows.
