...
> **Diagnostics use a different mechanism than this card's U8b.** U8a (the scroll/cursor `ProviderSet` rewiring) and the generic `VirtualLineSource`/`set-virtual-lines!` plumbing described below are live infrastructure (see `docs/LSP.md`'s "Virtual-line-aware scroll/cursor math" decision row), but diagnostics' actual inline summary reuses U9's `InlineDecoration` shape instead: end-of-line text via a second `InlayHintProvider` instance fed by `set-inline-diagnostics!`, plus a `gn`/`gp` dismiss-on-any-key popup (`show-popup! #:kind 'scrollable`) for the full message. See `docs/LSP.md`'s U8 entry and `runtime/plugins/core/lsp/diagnostics.scm`. The `VirtualLineSource` path below remains available for a future under-the-line feature (git blame, code lens) that wants whole synthetic rows.

**Goal** — virtual text lines under diagnostic lines (first real `VirtualLineSource`), which **triggers the deferred editor rewiring** from `docs/LSP.md`'s decision *Virtual-line-aware scroll/cursor math*: thread `&ProviderSet` through the cursor/scroll math so virtual rows count.
...
**Read first** — `docs/LSP.md`'s decision row (verbatim constraints); `format::display_rows_for_line` + `RowsBreakdown` tests (engine); `VirtualLineSource` contract in `providers.rs` (called during scroll/cursor accounting, not just render — must be a cheap lookup).
