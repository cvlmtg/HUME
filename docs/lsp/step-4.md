...
```
`diag-jump`: `(diagnostics-for-buffer bid)` (already sorted) → compare the cursor's char offset (`(stdlib/cursor-char-index (current-selections))` — `core:stdlib`) against each entry's `"start"` → first entry strictly after / last strictly before (wrap around, report "no diagnostics" when empty) → `(goto-location! (list bid line col))` using the entry's `"line"`/`"col"` fields (shape 2 — char-indexed, no encoding involved). Drawer rows: severity glyph + first message line.
