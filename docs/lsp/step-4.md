...
**Deliverables**
1. **Manifest** — `runtime/plugins/core/lsp/plugin.scm`: no self-activation logic (that's the user's call in their own `init.scm`, via `declare-plugin` or `load-plugin` — see the plugin README's Setup section for the supported `#:languages`/`#:commands` triggers and the `#:events '(on-lsp-attach)`-alone footgun). Multi-file layout mimicking `plum` (`plugin.scm` + `lib.scm` + per-feature files via relative `require`).
