...
**Status: A1 (the widget/render-layer rename pass) and A2's session-token/multi-source-merge core have landed. A2's own store/module relocation (`editor/lsp/completion/` → alongside the minibuffer completers) was carved back out of A2 — it now rides with the insert-mode/minibuffer completion-system merge instead, where the module's final shape is decided once. A3–A4 (source plugin, buffer-words) are design only — not scheduled.** This document is the single place to resume from; it assumes the reader has *no* memory of the exploration that produced it.
...
- **Generic widgets** (all in `host_impl/ui.rs` + `hume-ui/src/popup.rs` + `hume-ui/src/drawer.rs`):
  ...
  - `(show-drawer-list! items on-select)` / `(close-drawer!)` — `DrawerLayer { items, selected, scroll, callback }`, bottom chrome band, stays open across Enter (callback may fire repeatedly, `#f` on close), `drawer_input` (`input_stack/drawer.rs`). Rows are pre-formatted display strings; "Rust never interprets row content."
...
**Store/module relocation is deferred past A2, not a standalone rename.** Since A1, the session's *owner* is `CompletionLayer` on the editor's input stack (it dies with the layer — Esc, `:lsp-stop`, or Insert ending any other way — with no deferred-dismiss bookkeeping needed to get there). A2's session-token/multi-source-merge work has since landed with the store's module still at `editor/lsp/completion/` — moving `accept.rs` out of the `editor::lsp` subtree turned out to force several `pub(in crate::editor::lsp)` items wider for no reason A2 itself needed, so the move was carved out and folded into the insert-mode/minibuffer completion-system merge instead, where accept's own target (buffer vs. minibuffer) decides the module's final shape in one step rather than two. `CompletionSession`/`CompletionItem` are already generic names, so nothing about A2's own work depended on the move.
