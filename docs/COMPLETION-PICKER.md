...
**Status: A1 (the widget/render-layer rename pass), A2's session-token/multi-source-merge core, and the store/module relocation (`editor/lsp/completion/` → `editor/completion/`, alongside the minibuffer completers) have all landed. A3–A4 (source plugin, buffer-words) are design only — not scheduled.** This document is the single place to resume from; it assumes the reader has *no* memory of the exploration that produced it.
...
`hume-editor/src/editor/completion/` (five pure completer functions, each `complete_x(input, cursor, ctx) -> CompletionResult`: `complete_command`, `complete_buffer_name`, `complete_theme`, `complete_path`, `complete_set` — the `Completer` trait these used to implement is gone, since every real call site dispatched by a hardcoded match, never polymorphically; dispatched by that same match in `complete_minibuf`, `input_stack/command.rs`; prefix matching only; rendered by the generic `PopupOverlay` via `resolve_menu`, the same widget the selection menu and LSP completion menu use). It shares no types with the insert-mode stack, and `ROADMAP.md` already records its own future direction ("Steel builtin to register custom completers … core does prefix matching only"). Neither this design nor the picker (`core:pickers`) builds on it, and neither changes it.
