...
**Completion input handling — `hume-editor/src/editor/mappings/completion_menu.rs`:** the `Completion` overlay layer's own handler, not Insert-mode code — `dispatch_at` routes to it whenever `Completion` is the topmost layer.
...
- **Generic widgets** (all in `host_impl/ui.rs` + `hume-ui/src/popup.rs` + `hume-ui/src/drawer.rs`):
  ...
  - `(show-menu! items on-select)` / `(close-menu!)` — `MenuModel { items, selected, callback }`; callback fires exactly once (selection or dismissal); pushed as its own layer on the editor's input stack, gated on the mode layer being `Base` — a menu open under Insert or a minibuf mode has no way to route its own keys, and the completion menu owns Insert's overlay slot regardless. Keys handled by `Editor::menu_input` (`mappings/widgets.rs`).
  - `(show-drawer-list! items on-select)` / `(close-drawer!)` — `DrawerModel { items, selected, scroll, callback }`, bottom chrome band, stays open across Enter (callback may fire repeatedly, `#f` on close), `Editor::drawer_input`. Rows are pre-formatted display strings; "Rust never interprets row content."
...
`hume-editor/src/editor/completion/` (`Completer` trait: pure `complete(input, cursor, ctx) -> CompletionResult`; implementors `CommandCompleter`, `BufferNameCompleter`, `ThemeCompleter`, `PathCompleter`, `SetCompleter`; dispatched by a hardcoded match in `complete_minibuf`, `mappings/command_mode.rs`; prefix matching only; rendered by the bespoke statusline-anchored `MinibufCompletionOverlay` in `hume-ui/src/completion_overlay.rs`). It shares no types with the insert-mode stack, and `ROADMAP.md` already records its own future direction ("Steel builtin to register custom completers … core does prefix matching only"). Neither this design nor the picker (`core:pickers`) builds on it, and neither changes it.
...
4. `update_filter`'s rank key becomes `(score, source_priority, sort_text, index)`. Exact position of `source_priority` in the key: see Q-A3.
...
**Store/module relocation belongs to A2, not a standalone rename.** Since A1, the session's *owner* is `InputLayer::Completion` on the editor's input stack (it dies with the layer — Esc, `:lsp-stop`, or Insert ending any other way — with no deferred-dismiss bookkeeping needed to get there), but the store's module still lives at `editor/lsp/completion/`, and the store still parses `lsp_types::CompletionItem` directly. Relocating the module onto `editor/completion_session.rs` is real estate that only earns its keep once a second source exists, so it's **A2**'s job (session token + multi-source merge), not a rename done ahead of it. `CompletionSession`/`StoredCompletionItem` are already generic names.
