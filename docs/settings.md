...
| `wrap-mode` | `none` \| `soft[:N]` \| `word[:N]` \| `indent[:N]` | `indent` | Soft line wrapping for newly opened panes. `none` disables wrapping. The bare keyword (`soft`, `word`, or `indent`) wraps at the pane content width; appending `:N` wraps at column N. `indent` indents continuation lines to match the line's indent level. Wrapping is a per-pane view setting — this only seeds new panes; use `:wrap` to toggle it for the current pane |
