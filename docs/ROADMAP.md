...
- **`top_row_offset` counts display rows of `top_line`'s whole visual block, not content rows alone** — a virtual `before`/`after` row (from a `VirtualLineSource`) is an equal scroll unit alongside a real wrap row, so a virtual block anchored to buffer line 0 scrolls into view one row at a time instead of appearing/disappearing atomically or disagreeing between the renderer and cursor/scroll math. See `docs/GIT-DIFF.md`'s Phase 5b section.
