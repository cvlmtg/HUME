...
- [ ] Scroll and render format nearly the same visible range independently under soft wrap — `scroll_into_view`'s `RowMap` and the render pass's fresh one (`pane_render.rs`) each pay for their own pass over `[top..top+height]`, since `RowMap`'s cache is one line deep and dies with the map. Not a `FormatScratch` problem (a `FormatScratch` holds one line by construction — merging the scroll and render instances was considered and doesn't touch this). The real fix needs a cache surviving across both `RowMap`s, keyed on rope revision × resolved wrap width × decoration-store generation. `ensure_cursor_visible`'s own forward+backward double walk (the other candidate here) was scoped down to a one-line cap fix, already shipped — see `hume-editor/src/editor/scroll.rs`'s `target` variable in `ensure_cursor_visible`.
