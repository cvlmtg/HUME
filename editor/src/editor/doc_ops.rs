...
use crate::editor::buffer::IntoApplyResult;
...
pub(crate) fn apply_doc_edit<R: IntoApplyResult>(
    ...
    let (new_sels, displaced, cs) = buffers.get_mut(buf_id).apply_edit(sels, cmd);
...
/// default state.
pub(crate) fn apply_doc_edit_grouped<R: IntoApplyResult>(
    buffers: &mut BufferStore,
    pane_state: &mut SecondaryMap<PaneId, SecondaryMap<BufferId, PaneBufferState>>,
    focused_pane_id: PaneId,
    buf_id: BufferId,
