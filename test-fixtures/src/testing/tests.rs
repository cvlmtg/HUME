...
fn parse_cursor_at_start() {
    ...
    assert_eq!(s.head(), co(0));
    assert_eq!(s.anchor(), co(0));
}
...
fn parse_cursor_at_end() {
    ...
    assert_eq!(buf.to_string(), "hello\n");
    assert_eq!(sels.primary().head(), co(5));
}
...
fn parse_cursor_in_middle() {
    ...
    assert_eq!(buf.to_string(), "hello\n");
    assert_eq!(sels.primary().head(), co(3));
}
...
fn parse_selection_near_end_of_buffer() {
    ...
    let s = sels.primary();
    assert_eq!(s.anchor(), co(3));
    assert_eq!(s.head(), co(6));
}
...
fn parse_two_cursors() {
    ...
    assert_eq!(sels.iter_sorted().next().unwrap().head(), co(0));
    assert_eq!(sels.iter_sorted().nth(1).unwrap().head(), co(3));
}
...
fn parse_two_forward_selections() {
    ...
    assert_eq!((s0.anchor(), s0.head()), (co(0), co(1)));
    assert_eq!((s1.anchor(), s1.head()), (co(3), co(4)));
}
...
fn parse_cursor_on_only_newline() {
    ...
    assert_eq!(sels.primary().head(), co(0));
}
...
fn serialize_cursor_at_end() {
    ...
    let buf = BufferText::from("hello");
    let sels = SelectionSet::single(Selection::collapsed(co(5)));
...
fn serialize_backward_selection() {
    ...
    let buf = BufferText::from("hello");
    let sels = SelectionSet::single(Selection::new(co(3), co(0)));
...
fn serialize_forward_selection_head_at_eof() {
    ...
    let buf = BufferText::from("hello");
    let sels = SelectionSet::single(Selection::new(co(0), co(5)));
    assert_eq!(serialize_state(&buf, &sels), "-[hello\n]>");
}
