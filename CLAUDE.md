...
- **Display columns**: Every "how many terminal columns does this text occupy" computation goes through `hume_rope::width` (`tab_advance`, `grapheme_width`, `str_width`) — never a direct `unicode-width` call. Grapheme *indexing* (`hume-rope/src/grapheme.rs`) counts clusters, not cells; LSP wire positions (`hume-rope/src/position_encoding.rs`) count UTF-16 code units or bytes — neither is a display column.
  - **Drawing follows the same model**: text is written to the frame with `hume_engine::render::write_text_run`, never ratatui's `Buffer::set_string`. `set_string` measures by its own rule — it drops a grapheme holding a control character or measuring zero, and adds a cell for a halfwidth dakuten — so a field sized with `str_width` and drawn with `set_string` can reserve columns nothing lands in, or draw past what it reserved. `write_text_run` walks by `grapheme_width`, renders a cluster the terminal must not be shown as itself — a control character it would act on, an invisible one it would collapse — as its codepoint (`<200b>`, the form Vim and Emacs use, styled `ui.virtual.invisible`) rather than as a blank, since a bidi override that renders like a space is the Trojan Source attack, and takes a `right_edge` because `set_string` clips only at the terminal buffer and would otherwise bleed through a pane, lane, or box border. Box-drawing glyphs go through it too — they need none of the substitution, but an exemption buys nothing.
  - **Enforced**: `cargo test text_is_written_through_one_writer` (in `hume-editor/src/editor/lints/text_writer.rs`) fails the build on `set_string`/`set_stringn`/`set_symbol`/`set_char` anywhere in a workspace crate's `src/`. The only annotated lines are `write_text_run`'s own cell primitive and the blank-a-cell helper beside it.
