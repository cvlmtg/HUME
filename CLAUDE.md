...
**Mints** — `::new` (trusted, no rope check: the value is already known valid, e.g. read back from another `CharOffset`, or the result of an ASCII-only scan), `::checked` (validates against a `&Rope`, `None` past `len_chars()` — Steel builtin args, a restored session position), `Default` (offset 0). No `::clamped`/`::snapped`: a wire position clamps line-then-column (`hume_rope::position_encoding::wire_to_char`), and landing on a grapheme boundary is `hume_rope::grapheme::snap_to_cluster_start`'s job directly — both more specific than a blind `idx.min(rope.len_chars())` on an already-known `CharOffset` would be.

**Arithmetic** — `.index()` escapes into ropey, tree-sitter, or any other foreign coordinate system. `chars_since` gives a char count between two offsets (debug-asserts on inversion, not a `Sub` impl — a raw subtraction would silently wrap). `shift(delta: isize)` repositions by a signed delta already known to preserve grapheme alignment.

**Ranges** — `ExclusiveRange<CharOffset>`/`InclusiveRange<CharOffset>` (same module) name the two range conventions the codebase mixes: half-open (`ChangeSet`'s position-mapping API, viewport/diagnostic ranges, anything feeding `text.slice()`) vs. both-ends-covered (`Selection`, every text-object/bracket/quote/tag/search finder) — instead of a bare `(usize, usize)` meaning either. Both mint via `::new` plus field access plus `contains` — no `is_empty`, no `Into<Range<usize>>`; a caller feeding `text.slice()` builds the `Range<usize>` from the two `CharOffset`s directly. `InclusiveRange<CharOffset>` alone adds `end_exclusive` (`self.end.shift(1)`, not a raw `.index() + 1`) — the inclusive→exclusive crossing every such `text.slice()` call needs. `Selection::end_exclusive`/`content_end_exclusive` give that same bound without hand `+ 1` arithmetic.

**Enforced** — the compiler (see preamble). Two places a buffer position stays plain `usize`: the Steel/LSP host boundary (`hume_scripting::host::Host`, `hume-editor/src/editor/host_impl.rs`) — a documented FFI contract (`docs/LSP.md`) that Steel decodes as a raw integer regardless, converting to/from `CharOffset` immediately at that seam — and `hume_engine::types::Grapheme.char_offset`, which needs a `usize::MAX` sentinel for a virtual display line's cells (no buffer position at all) that `CharOffset` has no room to represent; the comparison sites in `hume-engine/src/style/mod.rs` and `display_lines.rs` document the trade-off.
...
**Enforced** — the compiler (see preamble). `clippy.toml`'s `disallowed-methods` bans `ropey::Rope::len_lines`/`RopeSlice::len_lines`/`line_to_char`/`char_to_line` workspace-wide; `hume-rope/src/lines.rs`'s `ropey_line_count` and its sibling line-start/line-count functions are the sanctioned wrappers, the file carrying one `#[allow(clippy::disallowed_methods)]` for all of them.
...
**Arithmetic** — unlike `CharOffset`, a display column is a genuine accumulator (formatting advances one grapheme's width at a time), so `DisplayLineCol`/`BufferLineCol` expose named arithmetic instead of forbidding it outright: `advance`/`cells_since`/`cells_since_saturating` on both, `abs_diff` on `DisplayLineCol` alone, `shift` on `BufferLineCol` alone — each with a single real caller. `CharCol` stays as strict as `CharOffset` (mint via `new`/`index()` only); `GraphemeCol` adds `from_number`/`number()` for its 1-based display form, but no arithmetic. `ByteCol` has one named exception, `advance` (a byte length folded onto a byte column — a tree-sitter edit's end position, a bracket match's end byte); every other operation on it stays as strict as `CharOffset`.

**Enforced** — the compiler (see preamble), plus one gap no type closes: a bare `usize`/`u16` named `col`/`column` in a crate with no column type of its own (`hume-grid`, `hume-platform`, `hume-scripting`, `hume-lsp`, `hume-ui`, `hume-decorations`) is invisible to the compiler, and nothing polices a *new* `FooColumn` CamelCase type actually naming a gutter widget rather than a coordinate. `hume-scripting/src/host.rs`'s `LocationDisplay.grapheme_col_or_wire: Option<usize>` is the sanctioned display-value exception below — its *name* is the only thing carrying that meaning. The same file's two bare `char_col: usize` FFI params are the addressing-unit case the same rule permits — a Steel builtin argument, never rendered.
...
| **Display line** | one visual line a buffer line occupies once soft wrap, provider-injected virtual lines, and tilde filler are accounted for | `hume-engine`'s `DisplayLine`/`DisplayLineKind`/`DisplayLineMap`/`DisplayLinePos` (`display_lines.rs`, `types.rs`) — the single flattened view every render/scroll/cursor/mouse/movement consumer walks |
...
The three coincide only by accident (an unwrapped, undecorated line's one display line happens to land on one terminal row) and the types never assume it. A terminal/pane cell row stays a bare `u16`/`usize` named `row`/`*_row` (see preamble's `x`-family) — never a display-line type; a display-line *count* or *address* (`DisplayLineMap::advance`'s `delta`, `DisplayLinePos`) is never spelled `row`.

**`DisplayLinePos { line, slot }`** addresses one display line: `line` names the buffer line, `slot` its index within that line's own visual block (`before`-virtuals, then the line's own wrap/content display lines, then `after`-virtuals, in order). `ViewportState::top_slot`/`top_line` (`hume-engine/src/pane.rs`) are this same pair. `slot` is display-line-exclusive vocabulary within `hume-engine`'s viewport/scroll machinery — a same-named but unrelated concept (how many panes share a split) lives in `hume-engine/src/pipeline/layout.rs` as `share`/`shares_along`, not `slot`, specifically so the two never collide under one name in the same crate (see "'engine' is overloaded", above). One carve-out from that same-crate exclusivity: `hume_engine::builtins::sign_column::Sign::slot` is a user-facing term (the gutter sign slot a plugin reserves via `register-sign-source!`) predating and unrelated to the display-line sense.
