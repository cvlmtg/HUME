...
**Arithmetic** — unlike `CharOffset`, a display column is a genuine accumulator (formatting advances one grapheme's width at a time), so `DisplayLineCol`/`BufferLineCol` expose named arithmetic instead of forbidding it outright: `advance`/`cells_since`/`cells_since_saturating` on both, `abs_diff` on `DisplayLineCol` alone, `shift` on `BufferLineCol` alone — `advance` is the per-grapheme workhorse of every format walk, the rest have a small handful of callers apiece; exact counts live at each method's own doc comment in `hume-rope/src/column.rs`, not duplicated here. `CharCol` stays as strict as `CharOffset` (mint via `new`/`index()` only); `GraphemeCol` adds `from_number`/`number()` for its 1-based display form, but no arithmetic. `ByteCol` has one named exception, `advance` (a byte length folded onto a byte column — a tree-sitter edit's end position, a bracket match's end byte); every other operation on it stays as strict as `CharOffset`.
