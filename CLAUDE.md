...
- **Testing**: Every editing command, text object, and selection operation must be tested. No untested commands. Core editing logic uses Helix-style state triples (`initial, ops, expected` with cursor/selection markers). Renderer uses `insta` inline snapshots.
