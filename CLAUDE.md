...
**Arithmetic** — `.index()` escapes into ropey, tree-sitter, or any other foreign coordinate system. `chars_since` gives a char count between two offsets (debug-asserts on inversion, not a `Sub` impl — a raw subtraction would silently wrap). `shift(delta: isize)` repositions by a signed delta already known to preserve grapheme alignment; `shift_saturating(delta: isize)` is the same shift clamped to 0 instead of panicking, for a delta that may legitimately exceed `self` (a cramped cursor near the buffer start, an undo/redo delta larger than the position it lands on). `retreat(n: usize)`/`retreat_saturating(n: usize)` are `shift`/`shift_saturating`'s unsigned-length counterparts, for the common case of retreating by a `usize` count (a removed run's length, a typed-char count) rather than a signed delta a caller would otherwise negate by hand.
