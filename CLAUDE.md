...
- **Named commands** (`src/edit.rs`, `src/motion.rs`) are pure `(Buffer, SelectionSet) -> (Buffer, SelectionSet)` functions. They have no knowledge of keys.
- **Keymaps** live in the Editor layer — not yet implemented.
