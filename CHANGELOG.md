...
### Plugins
- `picker!` gains `#:query` (prefills the input line, firing `#:on-query-change` once for a non-empty prefill), `#:on-query-change` (fires with the new query on every keystroke that changes it), and `#:debounce-ms` for live-requery pickers — a live grep, say, that re-runs its source per pattern instead of only locally filtering already-fetched rows. `#:debounce-ms` composes `#:on-query-change` with `debounce`/`picker-source-stop!`/`picker-replace!` for you: stop-and-clear runs immediately on every keystroke, while only the callback itself waits out the window. A live session (`#:on-query-change` set) also disables the picker's own fuzzy filter, since the query already selects what the source returns.
