...
- New `indent`/`unindent` (`>`/`<`) shift every line touched by a selection by one indent level (a count shifts by that many, e.g. `3>`), in the buffer's `tab-width`/`tab-style`. Blank and whitespace-only lines are left alone. Each touched line's whole indent is re-rendered to the new width rather than just prepended to or trimmed from, so `>` and `<` always exactly invert each other.
