...
- **Some checks are types, some are clippy, a few are still lints.** Raw grapheme stepping, raw line-count re-derivation, and untagged column arithmetic are compiler errors (`CharOffset`/`RopeyLine`/`ContentLine`/the column newtypes have no arithmetic trait impls — see `CLAUDE.md`'s invariants). Direct `unicode-width`/`ropey::len_lines` calls are `clippy::disallowed_methods` violations (`clippy.toml`, enforced by `scripts/test-all.sh`/CI). `arch-lints/` still holds genuine source-scanning tests for the handful of things neither a type nor clippy can express — a Steel `#f`-means-absent marker decoded somewhere other than `args.rs`, a `tempfile::tempdir()`/`std::env::set_var` outside a `TEST_GLOBALS` claim in the editor test tree, a plugin manifest drifting from its defined commands, and more. If one of those fires, the pattern it caught is real — fix the code, do not rename around the check.
