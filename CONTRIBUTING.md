...
- CI runs `cargo fmt --all -- --check` once, then build, the full test suite, doctests, and the theme editor's npm tests on Linux, macOS, and Windows, plus a Linux-only check that the theme editor's committed bundle matches a fresh build. All of it must be green.
...
- **An invariant is enforced by the strongest tool that fits — types, then clippy, then a lint — tried in that order.**
  1. **Types, when the mistake can be made to not compile.** Raw grapheme stepping, raw line-count re-derivation, and untagged column arithmetic are compiler errors: `CharOffset`/`RopeyLine`/`ContentLine`/the column newtypes have private fields and no arithmetic trait impls, so `pos + 1` or `col - 1` doesn't type-check (see `CLAUDE.md`'s invariants). A type needs no opt-out and can't be worked around, so it wins whenever the domain admits one.
  2. **Clippy, when the mistake is a specific call that must go through our wrapper instead.** `Rope::len_lines()` and `UnicodeWidthStr::width()` are foreign APIs returning a bare `usize` with no domain of ours to attach a type to — but the wrapper we want in their place (`hume_rope::lines`, `hume_rope::width`) does bookkeeping the raw call skips. `clippy.toml`'s `disallowed-methods` bans the raw calls workspace-wide, enforced by `cargo clippy --workspace --all-targets -- -D clippy::disallowed_methods` in `scripts/test-all.sh`/CI. This fits when the call has a nameable path and the real exceptions are few enough to mark individually with a local `#[allow(clippy::disallowed_methods)]`.
  3. **`arch-lints/`, when neither of the above can see the rule at all.** a plugin's `manifest.scm` command list drifting from its own `define-command!` calls, `runtime/init.scm.example` missing a newly added core plugin, a generated `runtime/scheme/*.scm` header hand-edited away from the template that regenerates it, a bare `tempfile::tempdir()` in the editor test tree bypassing `safe_tempdir()`. There's no Rust item here to hang a type or a clippy path on, so a scanner is what's left.
