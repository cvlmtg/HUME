...
fn copy_cursor_to_next_line() {
    ...
    let (text, sels) = parse_state("f-[o]>o\nbar\n");
    let sels_out = cmd_copy_selection_on_next_line(&text, sels, 1, MotionMode::Move);
...
fn copy_next_line_count_3() {
    ...
    assert_state!(
        "-[a]>\nb\nc\nd\ne\n",
        |(text, sels)| cmd_copy_selection_on_next_line(&text, sels, 3, MotionMode::Move),
        "-[a]>\n-[b]>\n-[c]>\n-[d]>\ne\n"
    );
}
...
fn copy_next_line_count_exceeds_buffer_clamps() {
    ...
    assert_state!(
        "-[a]>\nb\nc\n",
        |(text, sels)| cmd_copy_selection_on_next_line(&text, sels, 10, MotionMode::Move),
        "-[a]>\n-[b]>\n-[c]>\n"
    );
}
...
fn copy_next_line_count_3_multiple_cursors() {
    ...
    let (text, sels) = parse_state("f-[o]>-[o]>\nbar\nbaz\nqux\n");
    let sels_out = cmd_copy_selection_on_next_line(&text, sels, 3, MotionMode::Move);
...
fn copy_next_line_count_usize_max_returns_instantly() {
    ...
    let (text, sels) = parse_state("-[a]>\nb\nc\nd\ne\n");
    let sels_out = cmd_copy_selection_on_next_line(&text, sels, usize::MAX, MotionMode::Move);
...
fn copy_next_line_range_selection() {
    ...
    assert_state!(
        "-[hello]>\nworld\n",
        |(text, sels)| cmd_copy_selection_on_next_line(&text, sels, 1, MotionMode::Move),
        "-[hello]>\n-[world]>\n"
    );
}
...
fn copy_cursor_to_prev_line() {
    ...
    let (text, sels) = parse_state("foo\nb-[a]>r\n");
    let sels_out = cmd_copy_selection_on_prev_line(&text, sels, 1, MotionMode::Move);
    assert_eq!(sels_out.len(), 2);
...
fn copy_prev_line_count_3() {
    ...
    assert_state!(
        "a\nb\nc\nd\n-[e]>\n",
        |(text, sels)| cmd_copy_selection_on_prev_line(&text, sels, 3, MotionMode::Move),
        "a\n-[b]>\n-[c]>\n-[d]>\n-[e]>\n"
    );
}
...
fn copy_prev_line_count_exceeds_buffer_clamps() {
    ...
    assert_state!(
        "a\nb\n-[c]>\n",
        |(text, sels)| cmd_copy_selection_on_prev_line(&text, sels, 10, MotionMode::Move),
        "-[a]>\n-[b]>\n-[c]>\n"
    );
}
...
fn copy_prev_line_count_3_primary_lands_on_furthest_copy() {
    let (text, sels) = parse_state("a\nb\nc\nd\n-[e]>\n");
    let sels_out = cmd_copy_selection_on_prev_line(&text, sels, 3, MotionMode::Move);
...
fn copy_prev_line_count_usize_max_returns_instantly() {
    let (text, sels) = parse_state("a\nb\nc\nd\n-[e]>\n");
    let sels_out = cmd_copy_selection_on_prev_line(&text, sels, usize::MAX, MotionMode::Move);
    assert_eq!(sels_out.len(), 5); // original + one copy per remaining line
}
