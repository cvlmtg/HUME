...
#[test]
fn nearest_word_on_line_with_extra_word_char_selects_whole_run() {
    // Cursor on the '-' itself: with '-' configured as a word char, its own
    // class is `Word` (not `Punctuation`), so the fast path (cursor already
    // on a word/punct char) fires directly on the whole hyphenated run.
    assert_state!(
        "foo-[-]>bar\n",
        |(text, sels)| cmd_select_word_nearest_on_line(
            &text,
