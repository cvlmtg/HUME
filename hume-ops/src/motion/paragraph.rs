...
pub(super) fn goto_next_paragraph(text: &BufferText, head: usize) -> usize {
    ...
    let total = text.ropey_line_count();
...
pub(super) fn goto_prev_paragraph(text: &BufferText, head: usize) -> usize {
    let mut line = text.char_to_line(head);
    ...
    while line > 0 && is_empty_line(text, line - 1) {
        line -= 1;
    }
