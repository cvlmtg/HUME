...
use hume_ui::width::{ELLIPSIS, ELLIPSIS_WIDTH, text_width, truncate_text};
...
pub(in crate::statusline) fn statusline_display_path(editor: &HumeStatusline<'_>) -> String {
    let doc = editor.doc();
    doc.display_path()
        .map(str::to_owned)
...
pub(in crate::statusline) fn shorten_path_to_width(
    display: &str,
