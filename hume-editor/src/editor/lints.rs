...
mod tests {
    ...
    const SCAN_FILES: &[&str] = &[
        ...
        "src/editor/mappings/mod.rs",
        "src/editor/mappings/normal.rs",
        "src/editor/mappings/insert.rs",
        "src/editor/mappings/execute.rs",
        "src/editor/mappings/lazy.rs",
        "src/editor/mappings/search_mode.rs",
        "src/editor/mappings/select_mode.rs",
        "src/editor/mappings/command_mode.rs",
        "src/editor/mod.rs",
    ...
    fn no_self_mut_method_calls_in_editor_module() {
        ...
        assert!(
            ...
            "\nBorrow-disjoint discipline violated.\n\
             ...
             as separate parameters.  See `hume-editor/src/editor/lints.rs` for the \
             full rule.\n\
             Violations:\n{}\n",
            violations.join("\n"),
        );
    ...
    /// All mode transitions must go through `commands::set_mode` (defined in
    /// `src/editor/commands/mod.rs`), which atomically writes the field and
    /// enqueues `OnModeChange`.  Direct `state.mode =` / `.mode =` writes
    /// outside that file bypass the hook and break script-visible mode-change
    /// notifications.
    ///
    /// Opt-out: annotate the line with `// mode-funnel-exempt: <reason>`.
    ///
    /// Fail oracle: paste
    ///   `self.state.mode = Mode::Normal;`
    /// into any file other than `src/editor/commands/mod.rs` — this test must
    /// fail naming that line.
    #[test]
    fn mode_writes_go_through_funnel() {
        let manifest = std::env::var("CARGO_MANIFEST_DIR")
            .expect("CARGO_MANIFEST_DIR not set — run via `cargo test`");
        let root = std::path::Path::new(&manifest);

        // The single file that is allowed to contain direct `.mode =` writes.
        let allowed_file = "src/editor/commands/mod.rs";
        ...
        for path in &paths {
            ...
            for (lineno, line) in src.lines().enumerate() {
                ...
                if in_test_block { continue; }
                if trimmed.starts_with("//") { continue; }
                if line.contains("// mode-funnel-exempt:") { continue; }

                // Strip inline comment before matching to avoid false positives.
                let code = match line.find("//") {
                    Some(idx) => &line[..idx],
                    None => line,
                };

                // Matches `.mode = ` but not `.mode ==` (comparisons are fine).
                // Struct initializers (`mode:`) don't contain `.mode =` so they
                // are naturally excluded.
                if code.contains(".mode =") && !code.contains(".mode ==") {
                    violations.push(format!(
                        "  {rel}:{} — direct `.mode =` write outside funnel: {trimmed}",
                        lineno + 1,
                    ));
                }
            }
        ...
        assert!(
            violations.is_empty(),
            "\nDirect `state.mode =` / `.mode =` write found outside `commands::set_mode`.\n\
             All mode transitions must go through `commands::set_mode` in\n\
             `src/editor/commands/mod.rs`, which atomically updates the field and\n\
             enqueues the `OnModeChange` hook.  A direct write silently drops the\n\
             hook and breaks script-visible mode-change notifications.\n\
             Annotate with `// mode-funnel-exempt: <reason>` only when a direct\n\
             write is provably hook-irrelevant (e.g. EditorState struct initializer).\n\
             Violations:\n{}\n",
            violations.join("\n")
        );
    }
