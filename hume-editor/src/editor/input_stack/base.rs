...
pub(in crate::editor) fn base_input(ed: &mut Editor, _r: LayerRef, ev: InputEvent) {
    match ev {
        InputEvent::Key(key) => handle_normal(ed, key),
        InputEvent::Paste(text) => apply_normal_mode_paste(ed, &text),
        InputEvent::Mouse(mouse) => ed.base_mouse(mouse),
    }
}
...
fn handle_normal(ed: &mut Editor, key: KeyEvent) {
    ...
    // / Char('$') with Modifiers::SHIFT. DISAMBIGUATE_ESCAPE_CODES keeps the
    ...
    let key = if key.modifiers == Modifiers::SHIFT {
        if let KeyCode::Char(_) = key.code {
            KeyEvent::new(key.code, Modifiers::NONE)
        } else {
            key
        }
    } else {
    ...
    if let Some(wc) = ed.state.wait_char.take() {
        ...
        if let Some(ch) = arg {
            let count = ed.state.count.take();
            ed.state.pending_char = Some(ch);
            ...
            let extend = (ed.state.mode() == EditorMode::Extend) || wc.ctrl_extend;
            ed.execute_keymap_command(wc.cmd_name.clone(), count, extend);
        }
    ...
    if key.code == KeyCode::Escape {
        ed.state.pending_keys.clear();
        ed.state.count = None;
        ed.state.pending_ctrl_extend = false; // cancel any pending extend mode
        ed.state.macro_pending = None; // cancel any pending q/Q register-name prompt
    ...
    if let Some(pending) = ed.state.macro_pending.take() {
        match pending {
            MacroPending::Record => {
    ...
    if let Some(RegisterPrefix::Awaiting) = ed.state.register_prefix {
        ed.state.register_prefix = None;
        if let KeyCode::Char(ch) = key.code
            && key.modifiers == Modifiers::NONE
            && is_valid_register_name(ch)
        {
            ed.state.register_prefix = Some(RegisterPrefix::Selected(ch));
        }
    ...
    if ed.state.pending_keys.is_empty() && key.modifiers == Modifiers::NONE {
        match key.code {
            KeyCode::Char(d @ '0'..='9') if d != '0' || ed.state.count.is_some() => {
                ...
                let n = ed.state.count.unwrap_or(0) * 10 + digit;
                ed.state.count = Some(n.min(MAX_COUNT));
                return;
            }
            _ => {}
        }
    ...
    if ed.state.pending_keys.is_empty() && key.modifiers == Modifiers::NONE {
        match key.code {
            KeyCode::Char('Q') => {
                if let Some((reg, keys)) = ed.state.macro_recording.take() {
                    // Always allow stopping an in-progress recording, even if
                    // the user has navigated to a read-only buffer since starting.
                    ed.state.registers.write_macro(reg, keys);
                } else if !ed.state.is_replaying && !ed.focused_buffer_read_only() {
                    ed.state.macro_pending = Some(MacroPending::Record);
                }
                // During replay, or on a read-only buffer: silently ignore.
                return;
            }
            KeyCode::Char('q') => {
                if !ed.state.is_replaying
                    && ed.state.macro_recording.is_none()
                    && !ed.focused_buffer_read_only()
                {
                    // Replay: wait for the register-name key.
                    ed.state.macro_pending = Some(MacroPending::Replay);
                }
            ...
            KeyCode::Char('"') => {
                ...
                ed.state.register_prefix = Some(RegisterPrefix::Awaiting);
                return;
            }
            _ => {}
        }
    ...
    if ed.state.mode() == EditorMode::Extend
        && !ed.state.config.keymap.extend.is_empty()
        && !key.modifiers.contains(Modifiers::CONTROL)
    {
        let mut seq = ed.state.pending_keys.clone();
        seq.push(key);
        match ed.state.config.keymap.extend.walk(&seq) {
            WalkResult::Leaf(cmd) => {
                ed.state.pending_keys.clear();
                ed.state.pending_ctrl_extend = false;
                let count = ed.state.count.take();
                ed.execute_keymap_command(cmd.name.clone(), count, false);
                return;
            }
            WalkResult::Interior => {
    ...
    let (result, ctrl_extend) =
        if key.modifiers.contains(Modifiers::CONTROL) && ed.state.pending_keys.is_empty() {
            match ed.state.config.keymap.normal.walk(&[key]) {
                WalkResult::NoMatch if ed.kitty_enabled => {
                    ...
                    let result = ed.state.config.keymap.normal.walk(&ed.state.pending_keys);
                    let ctrl_extend = match &result {
                        ...
                        WalkResult::Interior => {
                            ed.state.pending_ctrl_extend = true;
                            true
                        }
                        WalkResult::Leaf(c) => ed
                            .state
                            .config
                            .registry
                            .get_mappable(c.name.as_ref())
                            .is_some_and(|r| r.is_extendable()),
                        WalkResult::WaitChar(wc) => ed
                            .state
                            .config
                            .registry
            ...
            ed.state.pending_keys.push(key);
            (
                ed.state.config.keymap.normal.walk(&ed.state.pending_keys),
                ed.state.pending_ctrl_extend,
            )
        };
        ...
        WalkResult::Leaf(cmd) => {
            ...
            let extend = extend
                && ed
                    ...
                    .is_some_and(|r| r.is_extendable());
            let count = ed.state.count.take();
            ed.execute_keymap_command(cmd.name.clone(), count, extend);
        }
