use super::*;
use crate::editor::tests::doubles::{VirtualLineBlock, no_providers};
...
fn down_no_wrap_clamps_at_last_real_line() {
    ...
    // already documented on `scroll_cursor_to_display_line` (z z / z j), and required so
...
/// repeated single-row wheel notches, in either wrap mode: `top_slot`
...
#[test]
fn down_reaches_every_display_line_of_an_after_last_line_block() {
...
/// `top_slot = 0; top_line += 1` unconditionally on overshoot, which
...
fn down_overshoot_past_after_last_line_clamps_not_resets() {
    ...
    for wrap in [WrapMode::None, WrapMode::Soft { width: 80 }] {
        ...
        vp.top_slot = 1; // already partway into the After block
