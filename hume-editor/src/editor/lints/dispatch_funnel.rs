...
//!
//! **Why `pub(in crate::editor)` on `MappableCommand` doesn't replace this
//! lint**: enum variants inherit their enum's visibility and cannot be
//! narrowed individually, so lowering `MappableCommand` from `pub(crate)` to
//! `pub(in crate::editor)` only shrinks the set of modules that can name the
//! type — every one of the ~279 files already under `crate::editor` can
//! still write `MappableCommand::Motion { fun, .. }` and skip
//! `run_dispatch_pipeline`'s bookkeeping. Only this scan (or a real
//! second-execution-path abstraction the type system can express, which
//! doesn't exist here) closes that.
//!
