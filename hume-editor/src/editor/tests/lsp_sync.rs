...
/// Sets up an editor with a `RecordingLspBackend`, a registered "rust"
/// server, and a real on-disk file matching (so `:e` triggers a genuine
/// attach through `lsp_attach_buffer`). Returns the editor, the buffer id,
/// and the shared notification log.
...
fn did_open_carries_full_text_and_language_id() {
    ...
    let log = log.borrow();
    assert_eq!(log.len(), 1, "exactly one didOpen, nothing else queued yet");
    let (method, params) = &log[0];
