...
/// type, so both directions of "declared resync, has a match arm" become
/// ordinary compiler checks instead of the runtime `debug_assert!`/hand-typed
/// test this replaces: a new `resync: true` key with no variant here fails
/// to compile at `resync_key`'s generated arm (`ResyncKey::$gname` doesn't
/// exist), and `resync_derived_state`'s `match` over this enum must be
/// exhaustive, so a variant with no arm there fails to compile too. Not
