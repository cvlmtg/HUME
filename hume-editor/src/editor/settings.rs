...
/// One global setting key declared `resync: true` in the `define_settings!`
/// invocation below — one variant per such key, named for its own `$gname`
/// field identifier. `resync_key` (generated by the macro, from the same
/// `resync: true` declarations) is the only way to produce a value of this
/// type, so both directions of "declared resync, has a match arm" are
/// ordinary compiler checks: a new `resync: true` key with no variant here
/// fails to compile at `resync_key`'s generated arm (`ResyncKey::$gname`
/// doesn't exist), and `resync_derived_state`'s `match` over this enum must
/// be exhaustive, so a variant with no arm there fails to compile too. Not
/// itself macro-generated — an enum's variant list can't be built from a
/// macro invocation in item position — but every real drift between it and
/// the `resync: true` declarations is still caught by the compiler through
/// `resync_key`'s generated body, not by this enum staying hand-in-sync on
/// its own.
///
/// Variants are the settings' own snake_case field identifiers, not
/// CamelCase: `resync_key`'s generated body reuses `$gname` verbatim as the
/// variant name, and converting case there would need a proc-macro
/// dependency (`paste` or similar) this project has no other use for.
///
/// `resync: false` is not a valid declaration — omit the `resync:` clause
/// entirely for a key with no derived-state effect. A literal `false` still
/// compiles (both directions above only check that the clause is present,
/// not its value), producing a real variant here and a real arm in
/// `resync_derived_state`, but one gated by a constant-`false` condition —
/// permanently dead code that both compiler checks report as "wired".
