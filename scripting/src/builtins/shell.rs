...
pub(crate) fn validate_new_path(
    ...
    let canonical_parent = platform::fs::canonicalize(parent).map_err(|e| {
        SteelErr::new(
            steel::rerrs::ErrorKind::Generic,
    ...
    match platform::fs::symlink_metadata(&canonical_dest) {
...
pub(crate) fn git_clone(
    ...
    let output = platform::process::git_clone(&url, &dest_path).map_err(|e| {
        SteelErr::new(
            steel::rerrs::ErrorKind::Generic,
    ...
    if !output.status.success() {
        ...
        steel::stop!(Generic =>
            ...
            platform::process::exit_code_str(output.status),
            stderr.trim());
    }
    Ok(SteelVal::Void)
}
...
pub(crate) fn git_pull(ctx: &mut SteelCtx, dir: String) -> Result<SteelVal, SteelErr> {
    ...
    let canonical = platform::fs::canonicalize(&dir_path).map_err(|e| {
        SteelErr::new(
            steel::rerrs::ErrorKind::Generic,
    ...
    let output = platform::process::git_pull_in(&canonical).map_err(|e| {
        SteelErr::new(
            steel::rerrs::ErrorKind::Generic,
...
pub(crate) fn git_clone_rev(
    ...
    let status = platform::process::git_clone_rev(&url, &dest_path, &rev).map_err(|e| {
        SteelErr::new(
            steel::rerrs::ErrorKind::Generic,
    ...
    if !status.success() {
        ...
        let _ = platform::fs::remove_dir_all(&dest_path);
        steel::stop!(Generic =>
            "git-clone-rev: clone of {url} @ {rev} failed ({})",
            platform::process::exit_code_str(status));
    }
    Ok(SteelVal::Void)
}
...
pub(crate) fn curl_fetch(
    ...
    if let Some(parent) = dest_path.parent() {
        platform::fs::create_dir_all(parent).map_err(|e| {
            SteelErr::new(
                steel::rerrs::ErrorKind::Generic,
    ...
    let status = platform::process::curl_fetch(&url, &dest_path).map_err(|e| {
        SteelErr::new(
            steel::rerrs::ErrorKind::Generic,
    ...
    if !status.success() {
        let _ = platform::fs::remove_file(&dest_path);
        steel::stop!(Generic =>
